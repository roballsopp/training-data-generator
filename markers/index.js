...
class Markers {
	static fromFile(markerPath, mapPath) {
		...
		return fsReadFile(markerPath)
			.then(parseMidi)
			.then(midiFile => new Markers(midiFile, midiMap))
	}
	...
	constructor(midiFile, midiMap) {
		this._ticksPerBeat = midiFile.header.ticksPerBeat;
		this._events = midiFile.tracks[0];
	...
	getSamplePosMap(sampleRate = 44100) {
		...
		for (let i = 0, l = this._events.length; i < l; i++) {
			...
			const numBeatsSinceLastEvt = evt.deltaTime / this._ticksPerBeat;

			elapsedSamples += (numBeatsSinceLastEvt * currentSampPerBeat);

			if (evt.type === 'setTempo') currentSampPerBeat = (evt.microsecondsPerBeat / MuS_PER_SECOND) * sampleRate;
				...
				const mappedNote = this._midiMap[evt.noteNumber]; // map 128 possible midi notes into NUM_ARTICULATIONS
